...
if not API_ID or not API_HASH or not BOT_TOKEN:
    ...
    print("Please set these in your render.yaml or deployment environment")
    sys.exit(1)
...
# Dynamic tokens that can be updated via commands
token_cp = os.getenv("TOKEN_CP", "eyJjb3Vyc2VJZCI6IjQ1NjY4NyIsInR1dG9ySWQiOm51bGwsIm9yZ0lkIjo0ODA2MTksImNhdGVnb3J5SWQiOm51bGx9r")
adda_token = os.getenv("ADDA_TOKEN", "eyJhbGciOiJIUzUxMiJ9..0QOuYFMkCEdVmwMVIPeETa6Kxr70zEslWOIAfC_ylhbku76nDcaBoNVvqN4HivWNwlyT0jkUKjWxZ8AbdorMLg")

# Store tokens in a file for persistence across restarts
TOKENS_FILE = os.getenv("TOKENS_FILE", "dynamic_tokens.json")

def load_dynamic_tokens():
    """Load dynamic tokens from file"""
    global token_cp, adda_token
    try:
        if os.path.exists(TOKENS_FILE):
            import json
            with open(TOKENS_FILE, 'r') as f:
                tokens = json.load(f)
                token_cp = tokens.get('token_cp', token_cp)
                adda_token = tokens.get('adda_token', adda_token)
                print(f"✅ Loaded dynamic tokens from {TOKENS_FILE}")
    except Exception as e:
        print(f"⚠️ Error loading dynamic tokens: {e}")

def save_dynamic_tokens():
    """Save dynamic tokens to file"""
    try:
        import json
        tokens = {
            'token_cp': token_cp,
        ...
        print(f"✅ Saved dynamic tokens to {TOKENS_FILE}")
    except Exception as e:
        print(f"⚠️ Error saving dynamic tokens: {e}")

# Load tokens on startup
load_dynamic_tokens()

# Image URLs from environment variables
photologo = os.getenv("PHOTO_LOGO", "https://tinypic.host/images/2025/02/07/DeWatermark.ai_1738952933236-1.png")
photoyt = os.getenv("PHOTO_YT", "https://tinypic.host/images/2025/03/18/YouTube-Logo.wine.png")
photocp = os.getenv("PHOTO_CP", "https://tinypic.host/images/2025/03/28/IMG_20250328_133126.jpg")
photozip = os.getenv("PHOTO_ZIP", "https://envs.sh/cD_.jpg")
    ...
    try:
        if len(message.command) < 2:
            await message.reply_text(
                "📝 **Usage:** `/add_user <id> [name]`\n\n"
                "**Examples:**\n"
                "• `/add_user 123456789` - Add user by ID only\n"
                "• `/add_user 123456789 John` - Add user with name\n\n"
                "💡 **Tip:** Name is optional for easier identification."
            )
            return

        new_user_id = int(message.command[1])
        user_name = " ".join(message.command[2:]) if len(message.command) > 2 else "Unknown"

        if new_user_id in AUTH_USERS:
            await message.reply_text(f"✅ User ID `{new_user_id}` is already authorized.")
        else:
            ...
            await message.reply_text(
                ...
                f"📝 **Name:** {user_name}\n"
                f"📊 **Total Authorized Users:** {len(AUTH_USERS)}"
            )
    except (IndexError, ValueError):
        await message.reply_text("❌ Please provide a valid user ID (numbers only).")
...
async def list_auth_users(client: Client, message: Message):
    ...
    user_list = '\n'.join(map(str, AUTH_USERS))  # Get authorized user IDs
...
async def remove_auth_user(client: Client, message: Message):
    ...
    try:
        if len(message.command) < 2:
            await message.reply_text(
                "📝 **Usage:** `/remove_user <id>`\n\n"
                "**Example:** `/remove_user 123456789`\n\n"
                "💡 **Tip:** Use `/users` to see all authorized user IDs."
            )
            return

        user_id_to_remove = int(message.command[1])
        if user_id_to_remove not in AUTH_USERS:
            await message.reply_text(f"❌ User ID `{user_id_to_remove}` is not in the authorized users list.")
        else:
            AUTH_USERS.remove(user_id_to_remove)
            await message.reply_text(
                f"✅ **User Removed Successfully!**\n\n"
                f"👤 **Removed User ID:** `{user_id_to_remove}`\n"
                f"📊 **Remaining Authorized Users:** {len(AUTH_USERS)}"
            )
    except (IndexError, ValueError):
        await message.reply_text("❌ Please provide a valid user ID (numbers only).")
...
@bot.on_message(filters.command("load_token_cp") & filters.private)
async def load_token_cp_handler(client: Client, m: Message):
    """Update token_cp dynamically"""
    if m.chat.id != OWNER:
        return await m.reply_text("❌ You are not authorized to use this command.")

    try:
        # Check if token is provided in the command
        if len(m.command) < 2:
            await m.reply_text(
                "📝 **Usage:** `/load_token_cp <new_token>`\n\n"
                "**Example:** `/load_token_cp eyJjb3Vyc2VJZCI6IjQ1NjY4NyI...`\n\n"
                "💡 **Tip:** Send the new token_cp value after the command."
            )
            return

        # Get the new token from command arguments
        new_token = " ".join(m.command[1:])

        # Validate token format (basic check)
        if len(new_token) < 10:
            await m.reply_text("❌ Invalid token format. Token seems too short.")
            return

        # Update global token_cp
        global token_cp
        old_token_preview = token_cp[:20] + "..." if len(token_cp) > 20 else token_cp
        token_cp = new_token
        ...
        await m.reply_text(
            f"✅ **Token CP Updated Successfully!**\n\n"
            f"🔄 **Old Token:** `{old_token_preview}`\n"
            f"🆕 **New Token:** `{new_token_preview}`\n\n"
            f"💾 Token saved to file for persistence across restarts.\n"
            f"⏰ Updated at: `{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}`"
        )

        # Log the update
        print(f"🔄 TOKEN_CP updated by user {m.chat.id} at {datetime.datetime.now()}")

    except Exception as e:
        await m.reply_text(f"❌ Error updating token: {str(e)}")

@bot.on_message(filters.command("load_adda_token") & filters.private)
async def load_adda_token_handler(client: Client, m: Message):
    """Update adda_token dynamically"""
    if m.chat.id != OWNER:
        return await m.reply_text("❌ You are not authorized to use this command.")
    ...
    try:
        ...
        if len(m.command) < 2:
            await m.reply_text(
                "📝 **Usage:** `/load_adda_token <new_token>`\n\n"
                "**Example:** `/load_adda_token eyJhbGciOiJIUzUxMiJ9...`\n\n"
                "💡 **Tip:** Send the new adda_token value after the command."
            )
            return
        ...
        # Update global adda_token
        global adda_token
        old_token_preview = adda_token[:20] + "..." if len(adda_token) > 20 else adda_token
        adda_token = new_token
        ...
        await m.reply_text(
            f"✅ **ADDA Token Updated Successfully!**\n\n"
            f"🔄 **Old Token:** `{old_token_preview}`\n"
            f"🆕 **New Token:** `{new_token_preview}`\n\n"
            f"💾 Token saved to file for persistence across restarts.\n"
            f"⏰ Updated at: `{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}`"
        ...
        # Log the update
        print(f"🔄 ADDA_TOKEN updated by user {m.chat.id} at {datetime.datetime.now()}")

    except Exception as e:
        await m.reply_text(f"❌ Error updating token: {str(e)}")

@bot.on_message(filters.command("show_tokens") & filters.private)
async def show_tokens_handler(client: Client, m: Message):
    """Show current token status"""
    if m.chat.id != OWNER:
        return await m.reply_text("❌ You are not authorized to use this command.")

    try:
        # Load token info from file if exists
        token_info = "No saved token info"
        if os.path.exists(TOKENS_FILE):
            import json
            with open(TOKENS_FILE, 'r') as f:
                tokens = json.load(f)
                updated_at = tokens.get('updated_at', 'Unknown')
                token_info = f"Last updated: {updated_at}"

        token_cp_preview = token_cp[:30] + "..." if len(token_cp) > 30 else token_cp
        adda_token_preview = adda_token[:30] + "..." if len(adda_token) > 30 else adda_token

        await m.reply_text(
            f"🔍 **Current Token Status**\n\n"
            f"🎯 **TOKEN_CP:**\n`{token_cp_preview}`\n\n"
            f"🎯 **ADDA_TOKEN:**\n`{adda_token_preview}`\n\n"
            f"📁 **File Info:** {token_info}\n\n"
            f"💡 **Commands:**\n"
            f"• `/load_token_cp <token>` - Update CP token\n"
            f"• `/load_adda_token <token>` - Update ADDA token\n"
            f"• `/show_tokens` - Show current tokens"
        )

    except Exception as e:
        await m.reply_text(f"❌ Error showing tokens: {str(e)}")
...
EDITED_FILE_PATH = os.getenv("EDITED_FILE_PATH", "./edited_output.txt")
...
async def txt_handler(client: Client, m: Message):
    await bot.send_message(m.chat.id, text= (
        ...
        f"➥ /add_user <id> [name] – Add User ID\n"
        f"➥ /remove_user <id> – Remove User ID\n"
        f"➥ /users – Total User List\n"
        f"▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰\n"
        f"🔑 𝗧𝗼𝗸𝗲𝗻 𝗠𝗮𝗻𝗮𝗴𝗲𝗺𝗲𝗻𝘁: **(OWNER)**\n\n"
        f"➥ /load_token_cp <token> – Update CP Token\n"
        f"➥ /load_adda_token <token> – Update ADDA Token\n"
        f"➥ /show_tokens – Show Current Tokens\n"
        f"▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰\n"
...
async def txt_handler(bot: Client, m: Message):  
            ...
            while retry_count < max_retries and not download_success:
                try:
                    ...
                    if "drive" in url:
                        ...
                        download_cmd = f"{cmd} -R 25 --fragment-retries 25"
                        result = os.system(download_cmd)
                        if result == 0 and os.path.exists(f'{name}.{ext}'):
                    ...
                    time.sleep(e.x)
                    # Don't increment retry_count for FloodWait, just retry
                    continue
                except Exception as e:
                    retry_count += 1
                    if retry_count < max_retries:
                        await bot.send_message(channel_id, f'🔄 **Retry {retry_count}/{max_retries}** - Download failed for `{str(count).zfill(3)} {name1}`\n**Error:** {str(e)}\n**Retrying in 5 seconds...**', disable_web_page_preview=True)
                        await asyncio.sleep(5)  # Wait before retry
                    else:
                        # All retries failed - stop the entire process
                        await bot.send_message(channel_id, f'❌ **PROCESS STOPPED** ❌\n\n**Failed after {max_retries} attempts:**\n**Name** =>> `{str(count).zfill(3)} {name1}`\n**Url** =>> {link0}\n\n<blockquote><i><b>Final Error: {str(e)}</b></i></blockquote>\n\n**📊 Summary before stopping:**\n🔗 Processed: {count-1} files\n✅ Successful: {count-1-failed_count} files\n❌ Failed: {failed_count+1} files', disable_web_page_preview=True)
                        return  # Stop the entire process

            # If we reach here, download was successful, move to next file
            if download_success:
    ...
    # Only show completion summary if we processed all files successfully
    success_count = count - 1 - failed_count  # count-1 because count is incremented after each file
    video_count = v2_count + mpd_count + m3u8_count + yt_count + drm_count + zip_count + other_count

    if raw_text7 == "/d":
        await bot.send_message(channel_id, f"<b>-┈━═.•°✅ ALL FILES COMPLETED ✅°•.═━┈-</b>\n<blockquote><b>🎯Batch Name : {b_name}</b></blockquote>\n<blockquote>🔗 Total URLs: {len(links)} \n┃   ┠🔴 Total Failed URLs: {failed_count}\n┃   ┠🟢 Total Successful URLs: {success_count}\n┃   ┃   ┠🎥 Total Video URLs: {video_count}\n┃   ┃   ┠📄 Total PDF URLs: {pdf_count}\n┃   ┃   ┠📸 Total IMAGE URLs: {img_count}\n\n💡 **Note:** Process completed all files with retry mechanism (3 attempts per file)</blockquote>\n")
    else:
        await bot.send_message(channel_id, f"<b>-┈━═.•°✅ ALL FILES COMPLETED ✅°•.═━┈-</b>\n<blockquote><b>🎯Batch Name : {b_name}</b></blockquote>\n<blockquote>🔗 Total URLs: {len(links)} \n┃   ┠🔴 Total Failed URLs: {failed_count}\n┃   ┠🟢 Total Successful URLs: {success_count}\n┃   ┃   ┠🎥 Total Video URLs: {video_count}\n┃   ┃   ┠📄 Total PDF URLs: {pdf_count}\n┃   ┃   ┠📸 Total IMAGE URLs: {img_count}\n\n💡 **Note:** Process completed all files with retry mechanism (3 attempts per file)</blockquote>\n")
...
if __name__ == "__main__":
    print("🚀 Starting Medusa Course Bot...")
    print(f"📱 Bot Token: {'✅ Set' if BOT_TOKEN else '❌ Missing'}")
    print(f"🔑 API ID: {'✅ Set' if API_ID else '❌ Missing'}")
    print(f"🔐 API Hash: {'✅ Set' if API_HASH else '❌ Missing'}")
    print(f"👤 Owner: {OWNER if OWNER else 'Not set'}")
    print(f"👥 Auth Users: {len(AUTH_USERS)} users")
    print("🔄 Starting bot...")
